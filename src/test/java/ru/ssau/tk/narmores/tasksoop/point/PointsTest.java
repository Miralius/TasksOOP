...
public class PointsTest {
    ...
    private boolean equalsApproximately(double firstNumber, double secondNumber) {
        return ((isNaN(firstNumber) && isNaN(secondNumber)) || (isInfinite(firstNumber) && isInfinite(secondNumber)) || Math.abs(firstNumber - secondNumber) < DOUBLE_EPSILON);
    }
    ...
    public void testLength() {
        assertEquals(Points.length(firstPoint), Math.sqrt(50), DOUBLE_EPSILON);
    }

    @Test
    public void testScalarProduct() {
        assertEquals(Points.scalarProduct(firstPoint, secondPoint), 1., DOUBLE_EPSILON);
    }
}